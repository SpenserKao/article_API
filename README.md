...
## _Abstract_
For the technical test on *article API*, this document outlines its requirements, and the design, development environment, setup/installation, execution, test for its implementation. Maintenance wise, eventhough of low frequency, some possibly adjustable parameters have been moved to configuration. Last but not the least, Wishlist outlines what can be improved, time permitting.<br/> <br/>
...
- For requirement#1 (R1) re POST request act, should consider checking whether there exists a redundant article has already exist; if so, should reject the request. Of course, the criteria of redundancy should be discussed: jointly Id, Title? Or more?
